 ...
 */
uint8_t S2LPFifoReadNumberBytesRxFifo(S2LPDriver *devp)
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, RX_FIFO_STATUS_ADDR, 1, &tmp);
  return tmp;
 ...
 */
uint8_t S2LPFifoReadNumberBytesTxFifo(S2LPDriver *devp)
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, TX_FIFO_STATUS_ADDR, 1, &tmp);
  return tmp;
 ...
 * @retval None.
 */
void S2LPFifoSetAlmostFullThresholdRx(S2LPDriver *devp, uint8_t cThrRxFifo)
{
  uint8_t tmp;
  
  s_assert_param(IS_FIFO_THR(cThrRxFifo));

  S2LPSpiReadRegisters(devp, FIFO_CONFIG3_ADDR, 1, &tmp);
  
  tmp &= ~RX_AFTHR_REGMASK;
  tmp |= cThrRxFifo;

  g_xStatus = S2LPSpiWriteRegisters(devp, FIFO_CONFIG3_ADDR, 1, &tmp);
 ...
 */
uint8_t S2LPFifoGetAlmostFullThresholdRx(S2LPDriver *devp)
{
  uint8_t tmp;

  g_xStatus = S2LPSpiReadRegisters(devp, FIFO_CONFIG3_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPFifoSetAlmostEmptyThresholdRx(S2LPDriver *devp, uint8_t cThrRxFifo)
{
  uint8_t tmp;

  s_assert_param(IS_FIFO_THR(cThrRxFifo));

  S2LPSpiReadRegisters(devp, FIFO_CONFIG2_ADDR, 1, &tmp);
  
  tmp &= ~RX_AETHR_REGMASK;
  tmp |= cThrRxFifo;

  g_xStatus = S2LPSpiWriteRegisters(devp, FIFO_CONFIG2_ADDR, 1, &tmp);
 ...
 */
uint8_t S2LPFifoGetAlmostEmptyThresholdRx(S2LPDriver *devp)
{
  uint8_t tmp;

  g_xStatus = S2LPSpiReadRegisters(devp, FIFO_CONFIG2_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPFifoSetAlmostFullThresholdTx(S2LPDriver *devp, uint8_t cThrTxFifo)
{
  uint8_t tmp;

  s_assert_param(IS_FIFO_THR(cThrTxFifo));

  S2LPSpiReadRegisters(devp, FIFO_CONFIG1_ADDR, 1, &tmp);

  tmp &= ~TX_AFTHR_REGMASK;
  tmp |= cThrTxFifo;

  g_xStatus = S2LPSpiWriteRegisters(devp, FIFO_CONFIG1_ADDR, 1, &tmp);
 ...
 */
uint8_t S2LPFifoGetAlmostFullThresholdTx(S2LPDriver *devp)
{
  uint8_t tmp;

  g_xStatus = S2LPSpiReadRegisters(devp, FIFO_CONFIG1_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPFifoSetAlmostEmptyThresholdTx(S2LPDriver *devp, uint8_t cThrTxFifo)
{
  uint8_t tmp;

  s_assert_param(IS_FIFO_THR(cThrTxFifo));

  S2LPSpiReadRegisters(devp, FIFO_CONFIG0_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, FIFO_CONFIG0_ADDR, 1, &tmp);
 ...
 */
uint8_t S2LPFifoGetAlmostEmptyThresholdTx(S2LPDriver *devp)
{
  uint8_t tmp;

  g_xStatus = S2LPSpiReadRegisters(devp, FIFO_CONFIG0_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPFifoMuxRxFifoIrqEnable(S2LPDriver *devp, SFunctionalState xNewState)
{
