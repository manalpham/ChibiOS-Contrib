 ...
 * @retval None.
 */
void S2LPGeneralSetExtRef(S2LPDriver *devp, ModeExtRef xExtMode)
{
  uint8_t tmp;
 ...
 *         This parameter can be: MODE_EXT_XO or MODE_EXT_XIN.
 */
ModeExtRef S2LPGeneralGetExtRef(S2LPDriver *devp)
{
  uint8_t tmp;
 ...
 */
uint8_t S2LPGeneralGetDevicePN(S2LPDriver *devp)
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, DEVICE_INFO1_ADDR, 1, &tmp);
  return tmp;
}
 ...
 * @retval S2LP version.
 */
uint8_t S2LPGeneralGetVersion(S2LPDriver *devp)
{
  uint8_t tmp;
  S2LPSpiReadRegisters(devp, DEVICE_INFO0_ADDR, 1, &tmp);
  return tmp;
}
...
* @retval None.
*/
void S2LPRadioSetExternalSmpsMode(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
  s_assert_param(IS_SFUNCTIONAL_STATE(xNewState));
  
  S2LPSpiReadRegisters(devp, PM_CONF4_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PM_CONF4_ADDR, 1, &tmp);
}
