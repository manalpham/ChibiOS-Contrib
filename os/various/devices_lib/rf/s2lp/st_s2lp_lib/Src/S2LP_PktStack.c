 ...
 * @retval None.
 */
void S2LPPktStackInit(S2LPDriver *devp, PktStackInit* pxPktStackInit)
{
  uint8_t tmpBuffer[6];
  ...
  S2LPSpiReadRegisters(devp, PROTOCOL1_ADDR, 1, &tmpBuffer[0]);
  tmpBuffer[0] |= AUTO_PCKT_FLT_REGMASK;
  S2LPSpiWriteRegisters(devp, PROTOCOL1_ADDR, 1, &tmpBuffer[0]);
  ...
  S2LPSpiReadRegisters(devp, PCKTCTRL3_ADDR, 1, &tmpBuffer[3]);
  tmpBuffer[3] &= ~(PCKT_FRMT_REGMASK | RX_MODE_REGMASK);
  tmpBuffer[3] |= PKT_FORMAT_STACK_CODE<<6;
  
  S2LPSpiReadRegisters(devp, PCKTCTRL2_ADDR, 2, &tmpBuffer[4]);
  ...
  S2LPSpiWriteRegisters(devp, PCKTCTRL6_ADDR, 6, tmpBuffer);
  ...
  if(pxPktStackInit->xCrcMode == PKT_NO_CRC) {
    S2LPPktStackFilterOnCrc(devp, S_DISABLE);
  }
  else {
    S2LPPktStackFilterOnCrc(devp, S_ENABLE);
  }
    /* Constellation map setting */
  S2LPSpiReadRegisters(devp, MOD1_ADDR, 1, tmpBuffer);
  tmpBuffer[0] &= ~G4FSK_CONST_MAP_REGMASK;
  S2LPSpiWriteRegisters(devp, MOD1_ADDR, 1, tmpBuffer);
}
 ...
 * @retval None.
 */
void S2LPPktStackGetInfo(S2LPDriver *devp, PktStackInit* pxPktStackInit)
{
  uint8_t tmpBuffer[6];

  S2LPSpiReadRegisters(devp, PCKTCTRL6_ADDR, 6, tmpBuffer);
 ...
 * @retval None.
 */
void S2LPPktStackAddressesInit(S2LPDriver *devp, PktStackAddressesInit* pxPktStackAddresses)
{
  uint8_t tmpBuffer[3];
  ...
  S2LPSpiReadRegisters(devp, PCKT_FLT_OPTIONS_ADDR, 1, &tmpBuffer[0]);
  ...
  S2LPSpiWriteRegisters(devp, PCKT_FLT_OPTIONS_ADDR, 1, &tmpBuffer[0]);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS2_ADDR, 3, tmpBuffer);
}
...
* @retval None.
*/
void S2LPPktStackGetAddressesInfo(S2LPDriver *devp, PktStackAddressesInit* pxPktStackAddresses)
{
  uint8_t tmpBuffer[3];

  S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS3_ADDR, 3, tmpBuffer);
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_OPTIONS_ADDR, 1, &tmpBuffer[0]);
 ...
 * @retval None.
 */
void S2LPPktStackSetFormat(S2LPDriver *devp)
{
  uint8_t tmp;
  ...
  S2LPSpiWriteRegisters(devp, PCKTCTRL3_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPPktStackSetPayloadLength(S2LPDriver *devp, uint16_t nPayloadLength)
{
  uint8_t tmpBuffer[2];
 ...
 */
uint16_t S2LPPktStackGetPayloadLength(S2LPDriver *devp)
{
  uint8_t tmpBuffer[2];
 ...
 */
uint16_t S2LPPktStackGetReceivedPktLength(S2LPDriver *devp)
{
  uint8_t tmpBuffer[2];
  uint16_t nPayloadLength;

  g_xStatus = S2LPSpiReadRegisters(devp, RX_PCKT_LEN1_ADDR, 2, tmpBuffer);
  nPayloadLength = (((uint16_t)tmpBuffer[0])<<8) | ((uint16_t)tmpBuffer[1]);
    nPayloadLength--;
...
* @retval None.
*/
void S2LPPktStackAutoAck(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
...
* @retval None.
*/
void S2LPPktStackNRetx(S2LPDriver *devp, uint8_t nRetx)
{
  uint8_t tmp;
...
*/
SFlagStatus S2LPPktStackGetTXAckRequest(S2LPDriver *devp)
{
  uint8_t tmp;
  
  S2LPSpiReadRegisters(devp, RX_PCKT_INFO_ADDR, 1, &tmp);
  tmp &= NACK_RX_REGMASK;
  tmp >>= 2;
...
* @retval None.
*/
void S2LPPktStackAckRequest(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
  s_assert_param(IS_SFUNCTIONAL_STATE(xNewState));
...
* @retval None.
*/
void S2LPPktStackPiggybacking(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
  s_assert_param(IS_SFUNCTIONAL_STATE(xNewState));
...
* @retval None.
*/
void S2LPPktStackSeqNumForReload(S2LPDriver *devp, uint8_t cReloadValue)
{
  uint8_t tmp;
  ...
  tmp |= (cReloadValue<<3);
  g_xStatus = S2LPSpiWriteRegisters(devp, PROTOCOL2_ADDR, 1, &tmp);
}
 ...
 */
uint8_t S2LPPktStackGetNReTx(S2LPDriver *devp)
{
  uint8_t tempRetValue;

  /* Reads the TX_PCKT_INFO register value */
  g_xStatus = S2LPSpiReadRegisters(devp, TX_PCKT_INFO_ADDR, 1, &tempRetValue);
