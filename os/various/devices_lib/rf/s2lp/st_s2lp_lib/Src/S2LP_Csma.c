 ...
 * @retval None.
 */
void S2LPCsmaInit(S2LPDriver *devp, SCsmaInit* pxCsmaInit)
{
  uint8_t tmpBuffer[4];
  ...
  S2LPSpiWriteRegisters(devp, CSMA_CONF3_ADDR, 4, tmpBuffer);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PROTOCOL1_ADDR, 1, &tmpBuffer[0]);
  ...
  * @retval None.
  */
void S2LPCsmaGetInfo(S2LPDriver *devp, SCsmaInit* pxSCsmaInit)
{
   uint8_t tmpBuffer[4];
   ...
   g_xStatus = S2LPSpiReadRegisters(devp, CSMA_CONF3_ADDR, 4, tmpBuffer);
 ...
 * @retval None.
 */
void S2LPCsma(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp, tmp2;
  ...
  if(xNewState==S_ENABLE)
  {
    ...
    S2LPSpiReadRegisters(devp, PM_CONF0_ADDR, 1, &tmp2);
    tmp2 |= SLEEP_MODE_SEL_REGMASK;
    S2LPSpiWriteRegisters(devp, PM_CONF0_ADDR, 1, &tmp2);
  }
  else
  {
    tmp &= ~CSMA_ON_REGMASK;
 ...
 */
SFunctionalState S2LPCsmaGetCsma(S2LPDriver *devp)
{
  uint8_t tmp;
 ...
 * @retval None.
 */
void S2LPCsmaPersistentMode(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
  s_assert_param(IS_SFUNCTIONAL_STATE(xNewState));
 ...
 */
SFunctionalState S2LPCsmaGetPersistentMode(S2LPDriver *devp)
{
  uint8_t tmp;
 ...
 * @retval None.
 */
void S2LPCsmaSeedReloadMode(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
  s_assert_param(IS_SFUNCTIONAL_STATE(xNewState));
 ...
 */
SFunctionalState S2LPCsmaGetSeedReloadMode(S2LPDriver *devp)
{
  uint8_t tmp;
 ...
 * @retval None.
 */
void S2LPCsmaSetBuCounterSeed(S2LPDriver *devp, uint16_t nBuCounterSeed)
{
  uint8_t tmpBuffer[2];
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, CSMA_CONF3_ADDR, 2, tmpBuffer);
 ...
 */
uint16_t S2LPCsmaGetBuCounterSeed(S2LPDriver *devp)
{
  uint8_t tmpBuffer[2];

  g_xStatus = S2LPSpiReadRegisters(devp, CSMA_CONF3_ADDR, 2, tmpBuffer);
 ...
 * @retval None.
 */
void S2LPCsmaSetBuPrescaler(S2LPDriver *devp, uint8_t cBuPrescaler)
{
  uint8_t tmp;
  s_assert_param(IS_BU_PRESCALER(cBuPrescaler));

  S2LPSpiReadRegisters(devp, CSMA_CONF1_ADDR, 1, &tmp);

  tmp &= ~BU_PRSC_REGMASK;
  tmp |= (cBuPrescaler<<2);

  g_xStatus = S2LPSpiWriteRegisters(devp, CSMA_CONF1_ADDR, 1, &tmp);
}
 ...
 */
uint8_t S2LPCsmaGetBuPrescaler(S2LPDriver *devp)
{
  uint8_t tmp;

  g_xStatus = S2LPSpiReadRegisters(devp, CSMA_CONF1_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPCsmaSetCcaPeriod(S2LPDriver *devp, SCsmaPeriod xMultiplierTbit)
{
  uint8_t tmp;
 ...
 */
uint8_t S2LPCsmaGetCcaPeriod(S2LPDriver *devp)
{
  uint8_t tmp;
 ...
 * @retval None.
 */
void S2LPCsmaSetCcaLength(S2LPDriver *devp, uint8_t xCcaLength)
{
  uint8_t tmp;
  s_assert_param(IS_CSMA_LENGTH(xCcaLength));

  S2LPSpiReadRegisters(devp, CSMA_CONF0_ADDR, 1, &tmp);

  tmp &= ~CCA_LEN_REGMASK;
  tmp |= xCcaLength;

  g_xStatus = S2LPSpiWriteRegisters(devp, CSMA_CONF0_ADDR, 1, &tmp);
 ...
 */
uint8_t S2LPCsmaGetCcaLength(S2LPDriver *devp)
{
  uint8_t tmp;

  g_xStatus = S2LPSpiReadRegisters(devp, CSMA_CONF0_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPCsmaSetMaxNumberBackoff(S2LPDriver *devp, uint8_t cMaxNb)
{
  uint8_t tmp;
 ...
 */
uint8_t S2LPCsmaGetMaxNumberBackoff(S2LPDriver *devp)
{
  uint8_t tmp;
