...
void S2LPRadioSearchFreqDevME(S2LPDriver *devp, uint32_t lFDev, uint8_t* pcM, uint8_t* pcE);
...
void S2LPRadioSearchFreqDevME(S2LPDriver *devp, uint32_t lFDev, uint8_t* pcM, uint8_t* pcE)
{
  ...
  S2LPSpiReadRegisters(devp, SYNT3_ADDR, 1, &tmp);
  if((tmp&BS_REGMASK) == 0) {
    bs = HIGH_BAND_FACTOR;
  }
  
  if(S2LPRadioGetRefDiv(devp)) {
    refdiv = 2;
  }
...
void S2LPRadioSearchChannelBwME(uint32_t lBandwidth, uint8_t* pcM, uint8_t* pcE)
{
  ...
  if(i!=0) {
    ...
    for(uint8_t j=0;j<3;j++) {
      if(((uint32_t)S_ABS(chfltCalculation[j]))<chfltDelta) {
...
uint8_t S2LPRadioInit(S2LPDriver *devp, SRadioInit* pxSRadioInitStruct)
{
  ...
  xState = S2LPRadioGetDigDiv(devp);
  if(((s_lXtalFrequency<DIG_DOMAIN_XTAL_THRESH) && (xState==S_ENABLE)) || ((s_lXtalFrequency>DIG_DOMAIN_XTAL_THRESH) && (xState==S_DISABLE))) {
    S2LPSpiCommandStrobes(devp, CMD_STANDBY);
    do{
      for(volatile uint8_t i=0; i!=0xFF; i++);
      S2LPRefreshStatus(devp);      // add a timer expiration callback
    }while(g_xStatus.MC_STATE!=MC_STATE_STANDBY);
    
    xState = (SFunctionalState)!xState;
    S2LPRadioSetDigDiv(devp, xState);
    
    S2LPSpiCommandStrobes(devp, CMD_READY);
    do{
      for(volatile uint8_t i=0; i!=0xFF; i++);
      S2LPRefreshStatus(devp);      // add a timer expiration callback
  ...
  S2LPSpiWriteRegisters(devp, IF_OFFSET_ANA_ADDR, 2, tmpBuffer);
  ...
  S2LPRadioSearchFreqDevME(devp, pxSRadioInitStruct->lFreqDev, &fdev_m, &fdev_e);
  S2LPSpiReadRegisters(devp, MOD1_ADDR, 1, &tmpBuffer[3]);
  ...
  S2LPSpiWriteRegisters(devp, MOD4_ADDR, 6, tmpBuffer);
  
  S2LPSpiReadRegisters(devp, PA_POWER0_ADDR, 3, &tmpBuffer[0]);
  ...
  S2LPSpiWriteRegisters(devp, PA_POWER0_ADDR, 3, &tmpBuffer[0]);
  
  /* Enable the freeze option of the AFC on the SYNC word */  
  S2LPSpiReadRegisters(devp, AFC2_ADDR, 1, &tmp8);
  tmp8 |= AFC_FREEZE_ON_SYNC_REGMASK; S2LPSpiWriteRegisters(devp, AFC2_ADDR, 1, &tmp8);
  
  return S2LPRadioSetFrequencyBase(devp, pxSRadioInitStruct->lFrequencyBase);
...
void S2LPRadioGetInfo(S2LPDriver *devp, SRadioInit* pxSRadioInitStruct)
{
  ...
  S2LPSpiReadRegisters(devp, SYNT3_ADDR, 4, tmpBuffer);
...
void S2LPRadioSetSynthWord(S2LPDriver *devp, uint32_t lSynthWord)
{
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, SYNT3_ADDR, 4, tmpBuffer);
}
...
{
  uint8_t tmpBuffer[4];
  g_xStatus = S2LPSpiReadRegisters(devp, SYNT3_ADDR, 4, tmpBuffer);
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, CHNUM_ADDR, 1, &cChannel);
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, CHNUM_ADDR, 1, &tmp);
  return tmp;
}
...
void S2LPRadioSetRefDiv(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, XO_RCO_CONF0_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, XO_RCO_CONF0_ADDR, 1, &tmp);
}
...
SFunctionalState S2LPRadioGetRefDiv(S2LPDriver *devp)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, XO_RCO_CONF0_ADDR, 1, &tmp);
...
void S2LPRadioSetDigDiv(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, XO_RCO_CONF1_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, XO_RCO_CONF1_ADDR, 1, &tmp);
}
...
SFunctionalState S2LPRadioGetDigDiv(S2LPDriver *devp)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, XO_RCO_CONF1_ADDR, 1, &tmp);
...
void S2LPRadioSetChannelSpace(S2LPDriver *devp, uint32_t lChannelSpace)
{
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, CH_SPACE_ADDR, 1, &tmp);
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, CH_SPACE_ADDR, 1, &tmp);
...
uint8_t S2LPRadioSetFrequencyBase(S2LPDriver *devp, uint32_t lFBase)
{
  ...
  tmp32 = S2LPRadioComputeSynthWord(lFBase, ((uint8_t)S2LPRadioGetRefDiv(devp)+1));
  ...
  S2LPSpiReadRegisters(devp, SYNTH_CONFIG2_ADDR, 1, &tmp);
  tmp &= ~PLL_PFD_SPLIT_EN_REGMASK;
  tmp |= (pfd_split<<2);
  S2LPSpiWriteRegisters(devp, SYNTH_CONFIG2_ADDR, 1, &tmp);
...
void S2LPRadioSetDatarate(S2LPDriver *devp, uint32_t lDatarate)
{
  ...
  S2LPSpiReadRegisters(devp, MOD4_ADDR, 3, tmpBuffer);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, MOD4_ADDR, 3, tmpBuffer);
...
uint32_t S2LPRadioGetDatarate(S2LPDriver *devp)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, MOD4_ADDR, 3, tmpBuffer);
  dr_m = (((uint16_t)tmpBuffer[0])<<8) | ((uint16_t)tmpBuffer[1]);
  dr_e = tmpBuffer[2]&DATARATE_E_REGMASK;
...
void S2LPRadioSetFrequencyDev(S2LPDriver *devp, uint32_t lFDev)
{
  ...
  S2LPRadioSearchFreqDevME(devp, lFDev, &uFDevM, &uFDevE);
  
  S2LPSpiReadRegisters(devp, MOD1_ADDR, 2, tmpBuffer);
  tmpBuffer[0] &= ~FDEV_E_REGMASK;
  tmpBuffer[0] |= uFDevE;
  tmpBuffer[1] = uFDevM;
  g_xStatus = S2LPSpiWriteRegisters(devp, MOD1_ADDR, 2, tmpBuffer);
}
...
uint32_t S2LPRadioGetFrequencyDev(S2LPDriver *devp)
{
  ...
  refdiv = (uint8_t)S2LPRadioGetRefDiv(devp) + 1;
  g_xStatus = S2LPSpiReadRegisters(devp, SYNT3_ADDR, 1, &tmp);
  if((tmp&BS_REGMASK) == 0) {
...
void S2LPRadioSetChannelBW(S2LPDriver *devp, uint32_t lBandwidth)
{
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, CHFLT_ADDR, 1, &tmpBuffer);
...
uint32_t S2LPRadioGetChannelBW(S2LPDriver *devp)
{
  ...
  /* Reads the channel filter register for mantissa and exponent */
  g_xStatus = S2LPSpiReadRegisters(devp, CHFLT_ADDR, 1, &tmpBuffer);
...
void S2LPRadioSetModulation(S2LPDriver *devp, ModulationSelect xModulation)
{
  ...
  S2LPSpiReadRegisters(devp, MOD2_ADDR, 1, &tmpBuffer);
  tmpBuffer &= ~MOD_TYPE_REGMASK;
  tmpBuffer |= xModulation;
  g_xStatus = S2LPSpiWriteRegisters(devp, MOD2_ADDR, 1, &tmpBuffer);
}
...
ModulationSelect S2LPRadioGetModulation(S2LPDriver *devp)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, MOD2_ADDR, 1, &tmpBuffer);
...
void S2LPRadioSetMaxPALevel(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, PA_POWER0_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PA_POWER0_ADDR, 1, &tmp);
}
...
*       power value.
*/
void S2LPRadioSetPALeveldBm(S2LPDriver *devp, uint8_t cIndex, int32_t lPowerdBm)
{
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, address, 1, &paLevelValue);
}
...
int32_t S2LPRadioGetPALeveldBm(S2LPDriver *devp, uint8_t cIndex)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, address, 1, &paLevelValue);
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PA_POWER0_ADDR, 1, &tmp);
...
void S2LPRadioSetAutoRampingMode(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, PA_POWER0_ADDR, 2, tmpBuffer);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PA_POWER0_ADDR, 2, tmpBuffer);
}
...
void S2LPRadioCalibrationVco(S2LPDriver *devp, SFunctionalState xAmplitudeCalibration, SFunctionalState xFrequencyCalibration)
{
  ...
  S2LPSpiReadRegisters(devp, VCO_CONFIG_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, VCO_CONFIG_ADDR, 1, &tmp);
}
...
{
  value <<= 4;  
  g_xStatus = S2LPSpiWriteRegisters(devp, VCO_CALIBR_IN2_ADDR, 1, &value);
}
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, VCO_CALIBR_IN1_ADDR, 1, &value);
}
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, VCO_CALIBR_IN0_ADDR, 1, &value);
}
...
void S2LPRadioAfcInit(S2LPDriver *devp, SAfcInit* xSAfcInit)
{
  ...
  S2LPSpiReadRegisters(devp, AFC2_ADDR, 1, &tmpBuffer[0]);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, AFC2_ADDR, 3, tmpBuffer);
}
...
void S2LPRadioGetAfcInfo(S2LPDriver *devp, SAfcInit* xSAfcInit)
{
  ...
  S2LPSpiReadRegisters(devp, AFC2_ADDR, 3, tmpBuffer);
...
void S2LPRadioSetIsiEqualizationMode(S2LPDriver *devp, SIsiEqu xSIsiMode)
{
  ...
  S2LPSpiReadRegisters(devp, ANT_SELECT_CONF_ADDR, 1, &tmp);
  tmp &= ~EQU_CTRL_REGMASK;
  tmp |= (((uint8_t)xSIsiMode)<<5);
  g_xStatus = S2LPSpiWriteRegisters(devp, ANT_SELECT_CONF_ADDR, 1, &tmp);
}
...
SIsiEqu S2LPRadioGetIsiEqualizationMode(S2LPDriver *devp)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, ANT_SELECT_CONF_ADDR, 1, &tmp);
...
void S2LPRadioSymClkRecoverInit(S2LPDriver *devp, SSymClkRecInit* xSSymClkRecInit)
{
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, CLOCKREC1_ADDR, 2, tmpBuffer);
}
...
void S2LPRadioGetSymClkRecoverInfo(S2LPDriver *devp, SSymClkRecInit* xSSymClkRecInit)
{
  ...
  S2LPSpiReadRegisters(devp, CLOCKREC1_ADDR, 2, tmpBuffer);
