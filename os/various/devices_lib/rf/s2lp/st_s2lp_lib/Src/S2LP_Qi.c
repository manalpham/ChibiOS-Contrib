 ...
 */
int32_t S2LPRadioGetRssidBm(S2LPDriver *devp)
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, RSSI_LEVEL_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPRadioSetRssiThreshdBm(S2LPDriver *devp, int32_t wRssiThrehsold)
{
  uint8_t tmp;
  s_assert_param(IS_RSSI_DBM(wRssiThrehsold));
  
  tmp = S2LPRadioRssidBmToReg(wRssiThrehsold);
  g_xStatus = S2LPSpiWriteRegisters(devp, RSSI_TH_ADDR, 1, &tmp);
}
...
* @retval None.
*/
void S2LPRadioRssiInit(S2LPDriver *devp, SRssiInit* xSRssiInit)
{
  uint8_t tmpBuffer[2];
  ...
  S2LPSpiReadRegisters(devp, RSSI_FLT_ADDR, 1, &tmpBuffer[0]);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, RSSI_FLT_ADDR, 2, tmpBuffer);
}
...
* @retval None.
*/
void S2LPRadioGetRssiInfo(S2LPDriver *devp, SRssiInit* xSRssiInit)
{
  uint8_t tmpBuffer[2];

  S2LPSpiReadRegisters(devp, RSSI_FLT_ADDR, 2, tmpBuffer);
...
* @retval None.
*/
void S2LPRadioCsBlanking(S2LPDriver *devp, SFunctionalState xCsBlank)
{
  uint8_t tmp;
...
* @retval None.
*/
void S2LPRadioAntennaSwitching(S2LPDriver *devp, SFunctionalState xAntennaSwitch)
{
  uint8_t tmp;
...
* @retval None.
*/
void S2LPRadioSetPqiCheck(S2LPDriver *devp, uint8_t cPqiLevel)
{
  uint8_t tmp;
  s_assert_param(IS_PQI_LVL_CHECK(cPqiLevel));

  S2LPSpiReadRegisters(devp, QI_ADDR, 1, &tmp);
  tmp &= ~PQI_TH_REGMASK;
  tmp |= (((uint8_t)cPqiLevel)<<1);
  S2LPSpiWriteRegisters(devp, QI_ADDR, 1, &tmp);
...
*/
SFlagStatus S2LPQiGetCs(S2LPDriver *devp)
{
  uint8_t tmp;

  S2LPSpiReadRegisters(devp, LINK_QUALIF1_ADDR, 1, &tmp);
...
* @retval None.
*/
void S2LPQiSetSqiCheck(S2LPDriver *devp, SFunctionalState xNewState)
{
  uint8_t tmp;
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, QI_ADDR, 1, &tmp);

}

/**
 * @brief  Sets the SQI threshold. The synchronization quality
 *         threshold is equal to SYNC_LEN - 2 * SQI_TH with SQI_TH = 0..7. When SQI_TH is 0 perfect match is required.
 *         It is recommended that the SQI check is always enabled.
 * @param  cSqiLevel parameter of the formula above.
 * 	   This parameter is a @ref SqiThreshold.
 * @retval None.
 */
void S2LPQiSetSqiThreshold(S2LPDriver *devp, uint8_t cSqiLevel)
{
  uint8_t tmp;
