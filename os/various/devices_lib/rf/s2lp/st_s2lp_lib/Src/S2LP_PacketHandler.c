...
uint8_t S2LPGetPacketFormat(S2LPDriver *devp)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, PCKTCTRL3_ADDR, 1, &tmp);
...
void S2LPSetPreambleLength(S2LPDriver *devp, uint16_t cPreambleLength)
{
  ...
  S2LPSpiReadRegisters(devp, PCKTCTRL6_ADDR, 1, &tmpBuffer[0]);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKTCTRL6_ADDR, 2, tmpBuffer);
...
uint16_t S2LPGetPreambleLength(S2LPDriver *devp)
{
  ...
  /* Reads the PCKTCTRL2 register value */
  g_xStatus = S2LPSpiReadRegisters(devp, PCKTCTRL6_ADDR, 2, tmpBuffer);
...
void S2LPSetSyncLength(S2LPDriver *devp, uint8_t cSyncLength)
{
  ...
  S2LPSpiReadRegisters(devp, PCKTCTRL6_ADDR, 1, &tmp);

  tmp &= ~SYNC_LEN_REGMASK;
  tmp |= (cSyncLength<<2);

  g_xStatus = S2LPSpiWriteRegisters(devp, PCKTCTRL6_ADDR, 1, &tmp);
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKTCTRL6_ADDR, 1, &tmp);
...
void S2LPPacketHandlerWhitening(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, PCKTCTRL1_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKTCTRL1_ADDR, 1, &tmp);
...
void S2LPPacketHandlerManchester(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, PCKTCTRL2_ADDR, 1, &tmp);

  tmp &= ~MANCHESTER_EN_REGMASK;
  tmp |= (((uint8_t)xNewState)<<1);

  g_xStatus = S2LPSpiWriteRegisters(devp, PCKTCTRL2_ADDR, 1, &tmp);
...
* @retval None.
*/
void S2LPSetSyncWords(S2LPDriver *devp, uint32_t lSyncWords, uint8_t cSyncLength)
{
  ...
  S2LPSpiWriteRegisters(devp, PCKTCTRL6_ADDR, 1, &tmp);
  
  /* Writes the new value on the PCKTCTRL2 register */
  g_xStatus = S2LPSpiWriteRegisters(devp, SYNC3_ADDR, 4, tmpBuffer);
...
void S2LPGetSyncWords(S2LPDriver *devp, uint32_t* lSyncWords, uint8_t* cSyncLength)
{
  ...
  *cSyncLength = tmp;
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, SYNC3_ADDR, 4, tmpBuffer);
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, RX_ADDRE_FIELD0_ADDR, 1, &tmp);
  return tmp;
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, RX_ADDRE_FIELD1_ADDR, 1, &tmp);
  return tmp;
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS0_ADDR, 1, &tmp);
  return tmp;
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS2_ADDR, 1, &tmp);
  return tmp;
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS1_ADDR, 1, &tmp);
  return tmp;
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS4_ADDR, 1, &tmp);
  return tmp;
}
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS3_ADDR, 1, &tmp);
  return tmp;
}
...
* @retval None.
*/
void S2LPSetRxSourceMask(S2LPDriver *devp, uint8_t mask)
{
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS4_ADDR, 1, &mask);
}
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS3_ADDR, 1, &address);
}
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS2_ADDR, 1, &address);
}
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS1_ADDR, 1, &address);
}
...
{
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS0_ADDR, 1, &address);
}
 ...
 * @retval None.
 */
void S2LPPacketHandlerSetRxMode(S2LPDriver *devp, DirectRx xDirectRx)
{
  uint8_t tmp;
  s_assert_param(IS_DIRECT_RX(xDirectRx));

  S2LPSpiReadRegisters(devp, PCKTCTRL3_ADDR, 1, &tmp);
  tmp &= ~RX_MODE_REGMASK;
  tmp |= (uint8_t)xDirectRx;
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKTCTRL3_ADDR, 1, &tmp);
 ...
 * @retval None.
 */
void S2LPPacketHandlerSetTxMode(S2LPDriver *devp, DirectTx xDirectTx)
{
  uint8_t tmp;
...
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(devp, PCKTCTRL1_ADDR, 1, &tmp);
 ...
 */
uint8_t S2LPPacketHandlerGetTransmittedSeqNumber(S2LPDriver *devp)
{
  uint8_t tempRegValue;

  /* Reads the TX_PCKT_INFO register value */
  g_xStatus = S2LPSpiReadRegisters(devp, TX_PCKT_INFO_ADDR, 1, &tempRegValue);
...
void S2LPPacketHandlerSetExtendedLenField(S2LPDriver *devp, SFunctionalState xExtendedLenField)
{
  ...
  S2LPSpiReadRegisters(devp, PCKTCTRL4_ADDR, 1, &tmp);
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKTCTRL4_ADDR, 1, &tmp);
}
...
void S2LPPacketHandlerSetCrcMode(S2LPDriver *devp, PktCrcMode xPktCrcMode)
{
  ...
  S2LPSpiWriteRegisters(devp, PCKTCTRL1_ADDR, 1, &tmp);
...
void S2LPPacketHandlerSetRxPersistentMode(S2LPDriver *devp, SFunctionalState xNewState)
{
  ...
  S2LPSpiReadRegisters(devp, PROTOCOL0_ADDR, 1, &tmp);
  if(xNewState == S_ENABLE) {
    tmp |= PERS_RX_REGMASK;
  }
  else {
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PROTOCOL0_ADDR, 1, &tmp);
}
...
void S2LPSetDualSyncWords(S2LPDriver *devp, uint32_t lSyncWords)
{
  ...
  g_xStatus = S2LPSpiWriteRegisters(devp, PCKT_FLT_GOALS3_ADDR, 4, tmpBuffer);
}
...
void S2LPGetDualSyncWords(S2LPDriver *devp, uint32_t* lSyncWords)
{
  ...
  g_xStatus = S2LPSpiReadRegisters(devp, PCKT_FLT_GOALS3_ADDR, 4, tmpBuffer);
  *lSyncWords = ((uint32_t)tmpBuffer[0]<<24) | ((uint32_t)tmpBuffer[1]<<16) | ((uint32_t)tmpBuffer[2]<<8) | ((uint32_t)tmpBuffer[3]);
}
